...
export const tests = [
    ...
    },
    {
        ...
        fn: () => {
            for (const type of ALL_TYPES) {
                const color = tokenColor(type);
                assert.ok(Array.isArray(color), `${type} should return array`);
                assert.equal(color.length, 3);
                for (const channel of color) {
                    assert.ok(Number.isInteger(channel) && channel >= 0 && channel <= 255, `channel ${channel} out of range for type ${type}`);
                }
            }
        },
    },
    {
        ...
        fn: () => {
            const fallback = tokenColor('unknown_token_xyz');
