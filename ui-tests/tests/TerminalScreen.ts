...
export class TerminalScreen {
  ...
  getLineTrimmed(row: number): string {
  ...
  containsText(text: string): boolean {
    for (let r = 0; r < this.rows; r++) {
      if (this.getLine(r).includes(text)) return true;
    }
    return false;
  }
   ...
   */
  containsTextInCols(text: string, colStart: number, colEnd: number): boolean {
    for (let r = 0; r < this.rows; r++) {
      const slice = this.getLine(r).slice(colStart, colEnd);
      if (slice.includes(text)) return true;
    }
    return false;
  }
   ...
   */
  private _parseEscape(data: string, i: number): number {
    const next = data[i + 1];
    if (next === '[') {
   ...
   */
  private _parseCSI(data: string, i: number): number {
    const start = i;
  ...
  private _handleCSI(params: string, cmd: string): void {
    if (cmd === 'H' || cmd === 'f') {
      ...
      if (n === 2 || n === 3) {
        ...
        for (let c = this.cursorCol; c < this.cols; c++) {
          if (this.cursorRow < this.rows) this.grid[this.cursorRow][c] = ' ';
        }
